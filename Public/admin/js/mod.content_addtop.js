...
ext  上传格式
**/
function flashupload(uploadid, name, textareaid, funcName,  controller, num,isPic,ext) {
 
    var setting = '&controller=' + controller + '&num=' + num+'&textareaid=' + textareaid +'&is_pic='+ isPic+'&ext='+ext ;
    var status = true;
   
    if (status) {
        Wind.use("artDialog", "iframeTools", function () {
            art.dialog.open(ROOT + '/index.php?a=swfupload&m=Admin&c=Upload'  + setting, {
                title: name,
                id: uploadid,
                width: '650px',
                height: '420px',
    ...
    if (!IsImg(filepath)) {
          Wind.use('artDialog', function () {
            art.dialog({
                ...
                width: 230,
                height: 150,
                content: '<a href="' + filepath + '" target="_blank"><img src="'+PUBLIC+'/images/icon/down.gif"> 单击打开</a>',
                lock: true
            });
        });
        return false;
...
function show_relation(modelid, id) {
    $.getJSON(ROOT + "/index.php?a=public_getjson_ids&m=Content&g=Content&modelid=" + modelid + "&id=" + id, function (json) {
        var newrelation_ids = '';
        if (json.data == null) {
            isalert('没有添加相关文章！');
            return false;
